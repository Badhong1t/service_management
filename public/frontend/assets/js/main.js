$(document).ready(function () {
  ...
  // Keep track of files to handle stray input issues
  let selectedFiles = [];
  ...
  fileInput?.addEventListener('change', () => {
      ...
      selectedFiles.forEach((file, index) => {
          ...
          filePreview.className =
              'flex items-center justify-between p-2 bg-white rounded-lg shadow-md w-full max-w-sm mb-2';
          ...
          previewImage.className = 'w-16 h-16 object-cover rounded-md mr-2';
          previewImage.style.display = 'none'; // Hide by default
  
          if (file.type.startsWith('image/')) {
              const reader = new FileReader();
              reader.onload = function (e) {
                  previewImage.src = e.target.result;
                  previewImage.style.display = 'block'; // Display image once it's loaded
              };
          ...
          // Append file preview elements to the preview container
      ...
      // Clear the file input to allow re-selecting the same file(s) if needed
      // fileInput.value = '';
  });
  
  // Attach selected files to the form when submitting
  ...
  form?.addEventListener('submit', (event) => {
      ...
      // Optionally, log or process formData for debugging
      console.log(...formData.entries());
  
      // Use formData to submit via AJAX if needed, or let the form submit normally
      // event.preventDefault(); // Uncomment to handle custom AJAX submission
  });
